...
export class HoraExtraService implements OnInit {
  ...
  async listarHoraExtra(solicitacoes: any): Promise<SolicitacaoHoraExtra[]> {
    ...
    try{      
      ...
      // .then((response) => {
      //   console.log(response, "chegou aquiiiiiii");
      //   return response;
      // })
      // .catch((error) => {
  ...
  */
  ...
  async listarFuncionarioSolicitante(): Promise<FuncionarioPorSolicitanteHe[]> {
    ...
    return await firstValueFrom(
      this.http.get<FuncionarioPorSolicitanteHe[]>(
        `${this.url}/funcionarios-por-solicitante`,
        { headers }
      )
    )
      .then((response) => {
  ...
  async incluir(
    solicitaHoraExtra: SolicitacaoHoraExtraIncluir
  ): Promise<SolicitacaoHoraExtra> {
    const headers = new HttpHeaders().append(
      'Content-Type',
      'application/json'
    );
  ...
  async alterar(
    solicitaHoraExtra: SolicitacaoHoraExtra
  ): Promise<SolicitacaoHoraExtra> {
    const headers = new HttpHeaders().append(
      'Content-Type',
      'application/json'
    ...
    const body = JSON.stringify(solicitaHoraExtra);
    return await firstValueFrom(
      this.http.put<SolicitacaoHoraExtra>(
        `${this.url}/${solicitaHoraExtra.id}`,
        body,
        { headers }
      )
    )
  ...
  ): Promise<SolicitacaoHoraExtraPorAprovador[] | null> {
  ...
  async solicitacoesPorSolicitante(
    solicitacoes: any
  ): Promise<SolicitacaoHoraExtra[]> {
    const headers = new HttpHeaders().set('Content-Type', 'application/json');
