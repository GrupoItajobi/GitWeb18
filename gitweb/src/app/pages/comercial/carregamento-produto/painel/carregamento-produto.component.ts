...
export class CarregamentoProdutoComponent implements OnInit, OnDestroy {
  ...
  selecionouCarregamento(info: any) {
    console.log('eventClickEditar: ação:' + info.acao + " - " + info.idCarregamento)
    let carregamento: CarregamentoFila = this.mapCarregamento.get(info.idCarregamento)!;
    if (carregamento) {
  ...
  async buscarCarregamento() {
    ...
    if (this.ue) {
      ...
      await this.painelCarregProdutoService.listarTodosAbertos(this.ue)
        .then(response => {
  ...
  popularEstoque() {
    ...
    this.saldoProduto?.forEach(s => {
      ...
      if (this.produtoDetalheSelecionado) {
        ...
        if (this.produtoSelecionado == s.tagProduto && this.produtoDetalheSelecionado == s.codigoProduto && s.codigoProduto && s.descricaoProduto) {
    ...
    this.cicloLiberadoCarregamento = this.simularSaldoEstoqueNoCiclo(this.cicloLiberadoCarregamento!);
    this.cicloEmCarregamento = this.simularSaldoEstoqueNoCiclo(this.cicloEmCarregamento!);
    this.cicloFilaCarregamento = this.simularSaldoEstoqueNoCiclo(this.cicloFilaCarregamento!);
  ...
  popularOpcoesProdutoDetalhe() {
    ...
    this.opcoesProdutoDetalhe = [];
    this.saldoProduto?.forEach(s => {
      if (this.produtoSelecionado == s.tagProduto && s.codigoProduto && s.descricaoProduto) {
    ...
    if (this.carregamentos) {
      this.carregamentos.forEach(c => {
        ...
        if (c.produto === this.produtoSelecionado) {
          ...
          } else if (c.checkpoint! == "Carregamento" || c.checkpoint! == "FilaCarregamento") {
      ...
      this.cicloFilaCarregamento = this.ordernarCarregamentos(this.cicloFilaCarregamento!)
      this.cicloEmCarregamento = this.ordernarCarregamentos(this.cicloEmCarregamento!)
      this.cicloLiberadoCarregamento = this.ordernarCarregamentos(this.cicloLiberadoCarregamento!)
      this.cicloBalSaida = this.ordernarCarregamentos(this.cicloBalSaida!)
      this.cicloDigExpedicao = this.ordernarCarregamentos(this.cicloDigExpedicao!)
  ...
  async alterarCarregamento(carregamento: CarregamentoFila) {
    await this.painelCarregProdutoService.alterar(carregamento)
      .then(request => {
        this.saving = false;
  ...
  async incluirCarregamento(carregamento: CarregamentoFila) {
    await this.painelCarregProdutoService.incluir(carregamento)
      .then(request => {
        this.saving = false;
        this.carregamentoEdicao = {};
        this.fecharDialogCreateCarregamento();
  ...
  onDragStartCarregamento(posicao: number, tagDragStart: string, carregamento: CarregamentoFila) {
    this.tagDragStart = tagDragStart;
    this.carregamentoEdicao = carregamento;
    this.posicaoInicial = posicao;
  }

  async onDropCarregamento(posicao: number, ciclo: string, carregamentos: CarregamentoFila[]) {
    if (ciclo == this.tagDragStart) {
      if (this.temPermissaoService.temTag('AlterarFila', this.ue)) {
        ...
        await this.painelCarregProdutoService.alterarOrdem(ordem)
          .then(() => {
        ...
        this.toastMessageService.showInfoMsg("Acesso para alterar Ordem da Fila não liberado!");

      }
    } else {
      this.toastMessageService.showInfoMsg("Não é permitida transferir da " + this.tagDragStart + " para " + ciclo);
    }
      ...
      let saida: CarregamentoSaida = (
        {
          ...
          idCarregamentoFila: this.dragCarregamento.carregamento.idCarregamentoFila
        }
      );
  ...
  async salvarRecarregamento(saida: CarregamentoSaida): Promise<boolean> {
    return await (this.painelCarregProdutoService.recarregamento(saida)
      .then(response => {
  ...
  async salvarEncerramento(saida: CarregamentoSaida): Promise<boolean> {
    console.log(saida)
    return await (this.painelCarregProdutoService.saida(saida)
      .then(response => {
        this.blockedPanelSaida = false;
        this.buscarCarregamento();
        return true;
  ...
  initFormAddCarregamento() {
    ...
    this.formAddCarregamento = this.formBuilder.group(
      {
        idCarregamentoFila: new FormControl(this.carregamentoEdicao.idCarregamentoFila),
        ue: new FormControl(this.carregamentoEdicao.ue),
        motoristaNome: new FormControl(this.carregamentoEdicao.motoristaNome, Validators.required),
        transportadora: new FormControl(this.carregamentoEdicao.transportadora),
        motoristaCPF: new FormControl(this.carregamentoEdicao.motoristaCPF, Validators.required),
        clienteNome: new FormControl(this.carregamentoEdicao.clienteNome, Validators.required),
        tipoVenda: new FormControl(this.carregamentoEdicao.tipoVenda, Validators.required),
        caminhaoPlaca: new FormControl(this.carregamentoEdicao.caminhaoPlaca, Validators.required),
        produto: new FormControl({ value: this.carregamentoEdicao.produto, disabled: this.formProdutoDisable }),
        ...
        quantidadeVolume: new FormControl(this.carregamentoEdicao.quantidadeVolume),
        obsComercialBloqueio: new FormControl(this.carregamentoEdicao.obsComercialBloqueio),
      });
